...
export const studentOperations = {
  ...
  async createStudent(studentData: Database['public']['Tables']['students']['Insert']) {
    const { data, error } = await supabase
      .from('students')
      .insert(studentData)
      .select()
...
export const certificateOperations = {
  // Create certificate
  async createCertificate(certificateData: Database['public']['Tables']['certificates']['Insert']) {
    const { data, error } = await supabase
      .from('certificates')
  ...
  // Update certificate
  async updateCertificate(id: string, updates: Database['public']['Tables']['certificates']['Update']) {
    const { data, error } = await supabase
      .from('certificates')
      .update(updates)
      .eq('id', id)
  ...
  async getCertificateStats() {
    ...
    const stats = {
      ...
      byCategory: {
        ...
        professional: data.filter(cert => cert.category === 'professional').length,
        skill: data.filter(cert => cert.category === 'skill').length,
        achievement: data.filter(cert => cert.category === 'achievement').length,
      }
    }
